...
Hustle.define('Modals.ReleaseHighlight', function ($) {
  ...
  var welcomeModalView = Backbone.View.extend({
    ...
    dismissModal: function dismissModal(e) {
      ...
      return $.post(ajaxurl, {
        action: 'hustle_dismiss_notification',
        name: 'release_highlight_modal_430',
        _ajax_nonce: this.$el.data('nonce')
      });
    }
  });
...
(function ($) {
  ...
  Optin.listingBase = Hustle.View.extend({
    ...
    openPreview: function openPreview(e) {
      var $this = $(e.currentTarget),
          id = $this.data('id'),
          type = $this.data('type');
...
Hustle.define('Mixins.Module_Design', function ($) {
  ...
  return _.extend({}, Hustle.get('Mixins.Model_Updater'), {
    ...
    resetPickers: function resetPickers(e) {
      var $el = $(e.target);
...
Hustle.define('Integration_Modal_Handler', function ($) {
  ...
  return Backbone.View.extend({
    ...
    close: function close(self) {
      var triggerSUIClose = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      // Kill AJAX hearbeat
      self.ajax.abort(); // When it's triggered by the overlay, SUI takes care of this.

      if (triggerSUIClose) {
        SUI.closeModal();
      }

      self.trigger('modal:closed'); // Refrest add-on list
...
(function ($) {
  ...
  Module.integrationsModal = {
    ...
    open: function open(e) {
      ...
      this.view.on('modal:closed', function () {
        return self.close();
      });
  ...
  Module.preview = {
    open: function open(id, type, $button) {
      var previewData = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
      var me = this,
          isInline = 'embedded' === type || 'social_sharing' === type;
      var $previewContainer = ''; // Fill a regular div if they're not inline modules.

      if (!isInline) {
        $previewContainer = $('#module-preview-container'); // If it doesn't exist already, add it.

        if (!$previewContainer.length) {
          $('.sui-wrap-hustle').append('<div id="module-preview-container"></div>');
          $previewContainer = $('#module-preview-container');
        }
      } else {
        // Use the preview modal for inline modules.
      ...
      $.ajax({
        ...
        if (res.success) {
          ...
          var $module = $previewContainer.find('.hustle-ui'); // Load select2 if this module has select fields.
          ...
          if ($module.find('.hustle-date').length) {
            var strings = optinVars.messages.days_and_months;
          ...
          setTimeout(function () {
            return HUI.maybeRenderRecaptcha($module);
          }, 1000);
        }
        ...
        if (isInline) {
          SUI.openModal('hustle-dialog--preview', $button[0], $previewContainer.find('.sui-box-header .hustle-modal-close')[0], true);
        } // Display the module.


        me.showModule(moduleId, data);
      }).always(function () {
        $('.sui-button-onload').removeClass('sui-button-onload');
      });
    },
    showModule: function showModule(id, data) {
      var el = '.hustle_module_id_' + id;

      if ('popup' === data.module_type) {
        var autohideDelay = '0' === String($(el).data('close-delay')) ? false : $(el).data('close-delay');
        HUI.popupLoad(el, autohideDelay);
      } else if ('slidein' === data.module_type) {
        var _autohideDelay = '0' === String($(el).data('close-delay')) ? false : $(el).data('close-delay');

        HUI.slideinLayouts(el);
        HUI.slideinLoad(el, _autohideDelay);
...
Hustle.define('SShare.Design_View', function ($) {
  ...
  return Hustle.View.extend(_.extend({}, Hustle.get('Mixins.Model_Updater'), Hustle.get('Mixins.Module_Design'), {
    ...
    render: function render() {
      ...
      this.createPickers(); // Trigger preview when this tab is shown.
