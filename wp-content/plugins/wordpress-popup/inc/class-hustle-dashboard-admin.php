...
class Hustle_Dashboard_Admin extends Hustle_Admin_Page_Abstract {
	...
	const HIGHLIGHT_MODAL_NAME = 'release_highlight_modal_430';
	...
	public function should_show_highlight_modal( $has_modules, $need_migrate ) {
		...
		if ( ! $was_highlight_dismissed || $is_force_highlight ) {
			...
			if ( $is_force_highlight || ( $has_modules && ! $need_migrate ) ) {
				wp_enqueue_script( 'hustle-wistia-json', '//fast.wistia.com/embed/medias/rxir9wutyt.jsonp', array(), '1.0.0', true );
				wp_enqueue_script( 'hustle-wistia', '//fast.wistia.com/assets/external/E-v1.js', array(), '1.0.0', true );
				return true;
			} else {
